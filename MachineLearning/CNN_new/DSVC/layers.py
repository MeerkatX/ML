...
def affine_backward(dout, cache):
    ...
    ###########################################################################
    N, dims, D, M = x.shape[0], x.shape[1:], w.shape[0], w.shape[1]
    dx = dout.dot(w.T).reshape(x.shape)  # dx=w
    dw = x.reshape([N, D]).T.dot(dout)  # dw=x
    db = np.ones(N).dot(dout)  # db=1
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dw, db
...
def dropout_forward(x, dropout_param):
    """
    Performs the forward pass for (inverted 反转) dropout.
    ...
    for n in range(N):
        ...
        for f in range(F):
            ...
            for h in range(H_):
                ...
                for wi in range(W_):
                    # dw=dout * x
                    dw[f] += dout[n][f][h][wi] * x_padding[n, :, h * stride:HH + h * stride,
                                                 wi * stride:WW + wi * stride]
                    # db=dout*1
                    db[f] += dout[n][f][h][wi]
                    # dx=dout*w
                    dx_padding[n, :, h * stride:HH + h * stride, wi * stride:WW + wi * stride] += dout[n][f][h][wi] * w[
                        f]
